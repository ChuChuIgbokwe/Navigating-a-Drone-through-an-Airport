...
class DroneNavigation():
    ...
    def Navigate(self):
        ...
        j = 0
        while j + 1 < len(path):
            self.path_tuples.append((path[j], path[j + 1]))
            j += 1
        ...
        if path_length_tracker == 1:
            ...
            while p < path_length_tracker:
                # initial traversal
                if p == 0:
                    init_data = g.get_edge_data(self.path_tuples[p][0], self.path_tuples[p][1])['angles']
                    if self.initial_pose[0][2] == '+':  # clockwise
                    ...
                    t = self._time_accelerating(road_dict[self.initial_pose[0][0]], abs(angle_d))
                    commands_list.append('GO {}'.format(round(abs(t), 1)))
                    commands_list.append('TRANSFER {} {}'.format(self.path_tuples[p][1], 0.1))
                    time_list.append(round(abs(t), 1))
                    time_list.append(0.1)
                    p += 1
                ...
                # intermediary traversals
                    ...
                    t = self._time_accelerating(road_dict[self.path_tuples[p][0]], abs(angle_d))
                    commands_list.append('GO {}'.format(round(abs(t), 1)))
                    commands_list.append('TRANSFER {} {}'.format(self.path_tuples[p][1], 0.1))
                    time_list.append(round(abs(t), 1))
                    time_list.append(0.1)
        ...
        print "Path Taken: ", self.path_tuples
...
if __name__ == "__main__":
    ...
    D.Navigate()
    # A = DroneNavigation(road_list,intersection_list,initial_pose=[('A',0,'-')],destination=[('A',2)])
    # A.Navigate()
    # B = DroneNavigation(road_list,intersection_list,initial_pose=[('A',355,'+')],destination=[('B',0)])
    # B.Navigate()
    # C = DroneNavigation(road_list, intersection_list, initial_pose=[('A', 355, '-')], destination=[('B', 0)])
    # C.Navigate()
